...
namespace {
    ...
    template<typename T>
    void
    resolveRawArg(rawRequest &raw,
                  const toml::table *tbl,
                  const char *name,
                  providedValue<T> rawRequest::*field) {
        auto &arg = raw.*field;
        if (!arg.wasProvided()) {
...
#define RESOLVE_RAW_ARG(name) resolveRawArg(raw, tbl, #name, &rawRequest::name)
    ...
    void
    resolveConfigValues(rawRequest &raw, const toml::table *tbl) {
...
#undef RESOLVE_RAW_ARG
    ...
    template<typename T>
    void
    copyProvidedOption(providedValue<T> &raw, const providedValue<T> &plugin_option) {
        if (plugin_option.wasProvided()) {
            raw.setProvided(plugin_option.value);
        }
    }
    ...
    applyPluginOptions(rawRequest &raw, const southboundPluginBenchmarkCommand &plugin) {
        ...
        if (plugin.name() == kBackendPosix) {
            ...
            setIntOption(
                raw.gusli_max_simultaneous_requests, options, "max_num_simultaneous_requests");
...
rawCommand::rawCommand()
    : options_{
          cliOption::option("config_file", "Config file", &request_.config_file),
          ...
          cliOption::option("initiator_seg_type",
                            "Initiator segment type",
                            &request_.initiator_seg_type),
          ...
          cliOption::flag("check_consistency",
                          "Enable consistency check",
                          &request_.check_consistency),
          ...
          cliOption::flag("recreate_xfer",
                          "Recreate transfers each iteration",
                          &request_.recreate_xfer),
          cliOption::option("large_blk_iter_ftr",
                            "Large block iteration factor",
                            &request_.large_blk_iter_ftr),
          ...
          cliOption::option("num_initiator_dev",
                            "Initiator device count",
                            &request_.num_initiator_dev),
          ...
          cliOption::option("progress_threads",
                            "Progress thread count",
                            &request_.progress_threads),
...
std::string_view
rawCommand::name() const {
    return "raw";
}

std::string_view
rawCommand::description() const {
    return "Run compatibility benchmark command";
}

const std::vector<cliOption> &
rawCommand::getOptions() const {
    return options_;
}

const rawRequest &
rawCommand::request() const {
    return request_;
}

scenario_type_t
rawCommand::scenarioType() const {
    return scenario_type_t::RAW;
}
...
int
rawCommand::run(southboundPluginBenchmarkCommand &) {
    return runRawRequest(request_);
}

bool
rawCommand::finalizeRequest(const southboundPluginBenchmarkCommand &plugin, std::string &error) {
    ...
    if (!request_.config_file.value.empty()) {
        try {
            ...
            oss << "Failed to load config file: " << request_.config_file.value << ": "
                << err.what();
            error = oss.str();
            return false;
        }
